...
function SidebarMain() {
  ...
  return (
    <div className="h-screen hidden lg:flex lg:w-[15rem]  flex-col overflow-scroll  scrollbar-hide bg-[radial-gradient(circle_at_bottom_left,_var(--tw-gradient-stops))] from-slate-800 via-purple-900 to-slate-800 ">
      <Link href={"/profile"} className="imageArea h-[10rem] w-full flex justify-center items-center">
            ...
            <div  className="flex flex-col">
              ...
              <div className={`ml-4 space-y-1 ${openDropdownIndex === index ? "block" : "hidden"}`}>
                {item.children.map((subItem, subIndex) => (
                  <Link
                    key={subIndex}
                    className="flex items-center gap-2 p-2 text-lg text-slate-800 hover:bg-gray-300 hover:rounded-xl font-bold"
                    href={subItem.link}
                  >
                    {subItem.subIcon}
                    {subItem.subName}
